...
struct JSONSchemaOptionBuilderTests {
  ...
  func objectOptions() throws {
    @JSONSchemaBuilder var sample: JSONSchemaRepresentable {
      JSONObject()
        .properties {
          JSONProperty(key: "property0") {
            JSONString()
          }
          JSONProperty(key: "property1") {
            JSONString()
          }
          JSONProperty(key: "property2") {
            JSONBoolean()
          }
          JSONProperty(key: "property3") {
            JSONNumber()
          }
        }
        .patternProperties {
          JSONProperty(key: "^property[0-1]$") {
            JSONString()
          }
        }
  ...
  @Test
  func objectOptionsProperty() throws {
    @JSONSchemaBuilder var sample: JSONSchemaRepresentable {
      JSONObject {
        JSONProperty(key: "key1", value: JSONInteger())
        JSONProperty(key: "key2") {
          JSONNumber()
        }
      }
    }
  ...
  @Test
  func supplementalObjectOptions() throws {
    @JSONSchemaBuilder var sample: JSONSchemaRepresentable {
      JSONObject()
    ...
    #expect(
      options == .options(
        ...
        unevaluatedProperties: .schema(.integer())
      )
    )
  }
  ...
  @Test
  func supplementalNumberOptions() throws {
    @JSONSchemaBuilder var sample: JSONSchemaRepresentable {
      JSONNumber()
        .multipleOf(1)
  ...
  @Test
  func supplementalArrayOptions() throws {
...
struct JSONSchemaAnnotationsBuilderTests {
  @Test
  func allAnnotations() async throws {
    @JSONSchemaBuilder var sample: JSONSchemaRepresentable {
      JSONNull()
        .title("Title")
    ...
    #expect(
      sample.schema.annotations == .annotations(
        title: "Title",
        description: "This is the description",
        default: "1",
      ...
      ==
        .object(
          ...
          .options(
            properties: [
              "productId": .integer(
                .annotations(description: "The unique identifier for a product")
              ),
              "productName": .string(
                .annotations(description: "Name of the product")
              )
            ]
          )
        )
