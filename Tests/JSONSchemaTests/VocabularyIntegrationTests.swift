...
struct VocabularyIntegrationTests {
  ...
  @Test func testSchemaWithoutValidationVocabulary() throws {
    ...
    #expect(
      result.isValid,
      "Validation should pass when validation vocabulary is omitted, so minimum constraint is ignored"
    )
    ...
    #expect(
      typeResult.isValid,
      "Type validation should also be ignored since validation vocabulary is omitted"
    )
  }
}
