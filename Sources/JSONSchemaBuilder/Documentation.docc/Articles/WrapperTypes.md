...
| ``JSONComponents/CompactMap`` | Like ``JSONComponents/Map`` but emits an error when the transform returns `nil`. |
...
| ``JSONComposition`` wrappers | ``JSONComposition/AnyOf``, ``JSONComposition/AllOf``, ``JSONComposition/OneOf``, and ``JSONComposition/Not`` compose multiple schemas. |
